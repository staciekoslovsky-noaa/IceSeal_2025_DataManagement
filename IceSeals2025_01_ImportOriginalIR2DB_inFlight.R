...
RPostgreSQL::dbSendQuery(
  con,
  "DELETE FROM surv_ice_seals_2025.tbl_detections_original_ir_inflight"
)
...
for (i in 1:nrow(folders)) {
  ...
  for (f in 1:length(ir_original)) {
    ...
    original_id <- RPostgreSQL::dbGetQuery(
      con,
      "SELECT max(id) FROM surv_ice_seals_2025.tbl_detections_original_ir_inflight"
    )
    ...
    original <- read.csv(
      paste(folders$folder_path[i], ir_file, sep = "\\"),
      skip = 2,
      header = FALSE,
      stringsAsFactors = FALSE,
      col.names = c(
    ...
    original <- original %>%
      ...
      mutate(
        detection_id = paste(
          ...
          flight,
          camera_view,
          paste(f, sprintf("%05d", detection), sep = "."),
          sep = "_"
        )
      ) %>%
      select(
    ...
    RPostgreSQL::dbWriteTable(
      con,
      c("surv_ice_seals_2025", "tbl_detections_original_ir_inflight"),
      original,
      append = TRUE,
      row.names = FALSE
    )
