# Ice Seals 2025: Import processed color detections to DB (Twin Otter)
...
RPostgreSQL::dbSendQuery(
  con,
  "DELETE FROM surv_ice_seals_2025.tbl_detections_processed_rgb WHERE flight < \'fl200\'"
)
...
for (i in 1:nrow(folders)) {
  if (i == 1) {
    ...
    processed_id <- RPostgreSQL::dbGetQuery(
      con,
      "SELECT max(id) FROM surv_ice_seals_2025.tbl_detections_processed_rgb"
    )
  ...
  processed <- read.csv(
    paste(folders$folder_path[i], rgb_validated, sep = "\\"),
    skip = 2,
    header = FALSE,
    stringsAsFactors = FALSE,
    col.names = c(
  ...
  processed <- data.frame(lapply(processed, function(x) {
    gsub("\\(trk-atr\\) *", "", x)
  })) %>%
    mutate(
      image_name = basename(sapply(
        strsplit(image_name, split = "\\/"),
    ...
    mutate(
      bound_left = as.integer(bound_left),
    ...
    mutate(
      detection_type = ifelse(
        detection_type == 'walrus' &
          (bound_bottom - bound_top) > 200 &
          (bound_right - bound_left) > 200,
        'walruses',
        detection_type
      )
    ) %>%
  ...
  RPostgreSQL::dbWriteTable(
    con,
    c("surv_ice_seals_2025", "tbl_detections_processed_rgb"),
    processed,
    append = TRUE,
    row.names = FALSE
  )
